...
# Uses Ray distributed executor for TP across nodes
...
CONTAINER="${CONTAINER:-vllm/vllm-openai:v0.17.0-cu130}"
...
ENV="$ENV -e RAY_memory_monitor_refresh_ms=0"  # Disable Ray memory monitor (UMA false positives)
ENV="$ENV -e VLLM_SLEEP_WHEN_IDLE=1"  # Reduce idle CPU from ~1600% to ~280%
...
VOLS="$VOLS -v /home/tom/llm/vllm-head-entrypoint.sh:/vllm-head-entrypoint.sh:ro"
VOLS_HEAD="$VOLS -v /home/tom/llm/vllm-serve-cmd.sh:/vllm-serve-cmd.sh:ro"
...
# structured output can't be disabled in v0.17.0 (no 'none' backend)
VLLM_ARGS="$VLLM_ARGS --host 0.0.0.0 --port 8000"

# Write serve command script (mounted into head container)
cat > /home/tom/llm/vllm-serve-cmd.sh <<SERVEOF
#!/bin/bash
exec vllm serve $MODEL $VLLM_ARGS
SERVEOF
chmod +x /home/tom/llm/vllm-serve-cmd.sh
scp /home/tom/llm/vllm-serve-cmd.sh spark-2:/home/tom/llm/vllm-serve-cmd.sh

# Worker joins Ray cluster
...
ssh spark-3 "docker run -d --name vllm-worker --gpus all --shm-size 16g \\
    ...
    $RDMA $ENV_WORKER $VOLS $CONTAINER \\
    -c 'ray start --address=$HEAD_IP:6379 --node-ip-address=$WORKER_IP --block'"

sleep 3

# Head: Ray head + wait for GPUs + vLLM serve
echo "=== Starting head on spark-2 ==="
ssh spark-2 "docker run -d --name vllm-head --gpus all --shm-size 16g \\
    --network host --ipc host --restart=no --entrypoint bash \\
    $RDMA $ENV_HEAD $VOLS_HEAD $CONTAINER \\
    /vllm-head-entrypoint.sh"
