#!/bin/bash
# Deploy Qwen3.6-35B-A3B-FP8 TP=2: spark-2 (head) + spark-3 (worker)
...
ssh spark-2 "cd ~/spark-vllm-docker && python3 run-recipe.py qwen3.6-35b-a3b-fp8 \
    ...
    --gpu-mem 0.50 -d -- --enforce-eager --generation-config auto --override-generation-config '{\"presence_penalty\":1.0}' --max-num-batched-tokens 8192"
