...
**Deploy:** `deploy-122b-fp8.sh` uses `run-recipe.py`. Pulls + rebuilds + copies image. `--no-build` skips (watchdog). Passes `-- --enforce-eager` (not in recipe, added in deploy script).
